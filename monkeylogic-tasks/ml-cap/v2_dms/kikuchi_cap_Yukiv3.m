...
delay_dist = linspace(750,1500,150);
...
targ_hold       = 100;  % Hold on chosen target (ms)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EPOCH 4: CHOICE
% Both targets appear; monkey saccades to chosen target
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

toggleobject([targ_left targ_right], 'eventmarker', ChoiceOn);

[choice, rt] = eyejoytrack('acquirefix', [targ_left targ_right], targ_window, 5000);


if ~choice
    trialerror(2); % no response
    toggleobject([fixation_point targ_left targ_right], 'status', 'off');
    idle(iti);
    return
end

% Map eyejoytrack output (1 or 2) back to object index
if choice == 1
    chosen_targ = targ_left;
else
    chosen_targ = targ_right;
end

% Hold on chosen target
ontarget = eyejoytrack('holdfix', chosen_targ, targ_window, targ_hold);

% if ~ontarget
%     trialerror(3); % broke fixation on chosen target
%     toggleobject([fixation_point targ_left targ_right], 'status', 'off');
%     idle(iti);
%     return
% end

% Turn off unchosen target
if ~ontarget
    %     trialerror(3); % broke fixation on chosen target
    toggleobject(~chosen_targ, 'status', 'off');
